...
def output_to_excel(comparison_rows, max_depth, base_filename, new_filename, output_filename='comparison_result.xlsx'):
    ...
    ws.freeze_panes = 'A3'  # Cell A3 will be the top-left cell of the scrollable area

    # Set column widths
    narrow_width = 5   # Narrow column width for hierarchy levels
    wide_width = 30    # Increased width for the last level columns
    ...
    for i in range(1, total_columns + 1):
        ...
        elif i == max_depth or i == total_columns:
            # Last column on left or right side (increase width)
            ws.column_dimensions[column_letter].width = wide_width
        else:
    ...
    for row_data in comparison_rows:
        ...
        # Apply fill and borders to the entire row, excluding the gap columns
...
def compare_ontologies(base_csv, new_csv, output_excel):
    # Read the hierarchies from CSV files
    base_paths, base_max_depth = read_hierarchy_csv(base_csv)
    new_paths, new_max_depth = read_hierarchy_csv(new_csv)
    ...
    output_to_excel(comparison_rows, max_depth, base_csv, new_csv, output_excel)
...
from hierarchy_converter import hierarchy_to_csv
from ontology_comparator import compare_ontologies

def main():
    # Input TXT files (the hierarchy files)
    base_txt = 'c1.txt'   # Replace with your actual TXT file paths
    new_txt = 'c2.txt'     # Replace with your actual TXT file paths

    # Intermediate CSV files (generated from the TXT files)
    base_csv = 'cco.csv'
    new_csv = 'core.csv'

    # Output Excel file
    output_excel = 'comparison_resultccocore.xlsx'

    # Step 1: Convert TXT hierarchy files to CSV files
    hierarchy_to_csv(base_txt, base_csv)
    hierarchy_to_csv(new_txt, new_csv)

    # Step 2: Compare the CSV hierarchy files and output Excel
    compare_ontologies(base_csv, new_csv, output_excel)

    print(f"Comparison complete. Results saved in {output_excel}")
